...
void BitmapClass::Settype(int _type)
{
	type = _type;
}
...
void BitmapClass::UpdatePosition()
{
	...
	// 벽 충돌 검사 (화면 밖으로 나가면 방향 반전)
	if (m_renderX < 0 || m_renderX > m_screenWidth - m_bitmapWidth)
	{
		m_speedX = -m_speedX;
	}
	if (m_renderY < 0 || m_renderY > m_screenHeight - m_bitmapHeight)
	{
		m_speedY = -m_speedY;
	}
}